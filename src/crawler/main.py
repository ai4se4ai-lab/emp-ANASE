...
def collect_platform(platform_name: str, config_path: str) -> List[Dict[str, Any]]:
    ...
    collectors = {
        'stackoverflow': 'stackoverflow_collector.StackoverflowCollector',
...
def generate_report(df: pd.DataFrame, dedup_result: Dict[str, Any], output_dir: str):
    ...
    if not df.empty:
        ...
        for platform, count in platform_counts.items():
            report.append(f"  {platform:15s}: {count:5d} records")

        report.append(f"  {'TOTAL':15s}: {len(df):5d} records")
        report.append("")
        ...
        for analogy_type, count in type_counts.items():
            report.append(f"  {analogy_type:15s}: {count:5d} mentions")
        report.append("")

        # SDLC phase distribution
        report.append("SDLC PHASE DISTRIBUTION")
        report.append("-" * 40)
        phase_counts = df['sldc_phase'].value_counts()
        for phase, count in phase_counts.items():
            report.append(f"  {phase:15s}: {count:5d} records")
        report.append("")
